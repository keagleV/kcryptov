...
#include <strings.h>
#include <unistd.h>
#include <kcryptov/kcv_kshellv_libcli.h>
#include <kcryptov/kcv_kshellv.h>
...
void run_child(int x) {
  ...
  cli = cli_init();
  cli_set_banner(cli, KCV_KSHELLV_BANNER);
  cli_set_hostname(cli, KCV_KSHELLV_HOSTNAME);
...
int ksv_shell_start() {
  ...
  addr.sin_port = htons(KCV_KSHELLV_LISTENING_PORT);
